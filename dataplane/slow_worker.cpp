...
SlowWorker::SlowWorker(SlowWorker&& other) :
		...
		tcp_connection_store_(other.tcp_connection_store_)
{
...
{
	if (tcp_connection_store_ == nullptr)
	{
		return;
	}
	...
	auto rings = tcp_connection_store_->GetRetransmitRings();
	for (auto [ring_retransmit_free, ring_retransmit_send] : rings)
	{
		while ((count_retransmit < YANET_PROXY_MAX_RETRANSMITS_PER_CYCLE) && (rte_ring_dequeue(ring_retransmit_send, (void**)&data) == 0))
		{
			// YANET_LOG_WARNING("SlowWorker try send retransmit\n");
			count_retransmit++;
			slow_worker_->IncrementCounter(data->counter_id);
			...
			if (mbuf == nullptr)
			{
				...
				slow_worker_->IncrementCounter(common::globalBase::static_counter_type::proxy_error_retransmit);
			}
			else
			{
				...
				SendToSlowWorker(mbuf, data->flow);
			}

			if (rte_ring_enqueue(ring_retransmit_free, (void**)&data) != 0)
            {
				// YANET_LOG_WARNING("Error enqueue in ring_retransmit_free\n");
				slow_worker_->IncrementCounter(common::globalBase::static_counter_type::proxy_error_retransmit);
			}
		}
	}
