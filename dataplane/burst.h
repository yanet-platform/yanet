...
public:
	...
	BasicBurst& operator=(BasicBurst&& other)
	{
		if (this != &other)
		{
			if (!empty())
			...
			if (!other.empty())
			{
				const std::uint16_t& count = other.Packets();
				memcpy(m_burst, other.begin(), sizeof(rte_mbuf*) * count);
				m_end = count;
	...
	BasicBurst& Rx(uint16_t port_id, uint16_t queue_id)
	{
		...
		m_begin = 0;
		m_end = rte_eth_rx_burst(port_id, queue_id, m_burst, BSize);
		return *this;
	}
	...
	uint16_t Free()
	{
		...
		rte_pktmbuf_free_bulk(m_burst + m_begin, count);
		m_begin = 0;
		m_end = 0;
		return count;
	}
