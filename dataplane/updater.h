...
private:
	...
	eResult Resize(const std::size_t size)
	{
		...
		next->copy(next_stats, stats_, *(pointer_.get()));
		stats_ = next_stats;
	...
	bool NeedToShrink()
	{
		return stats_.extended_chunks_size > 2 * ObjectType::extended_chunks_size_min &&
		       stats_.extended_chunks_count < stats_.extended_chunks_size / 4;
	}
};
