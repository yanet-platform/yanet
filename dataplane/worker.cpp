...
{
	metadata.size = 0;
	metadata.start_counters = common::sdp::GetStartData(YANET_CONFIG_COUNTERS_SIZE * sizeof(uint64_t), metadata.size);
	metadata.start_acl_counters = common::sdp::GetStartData(YANET_CONFIG_ACL_COUNTERS_SIZE * sizeof(uint64_t), metadata.size);
	metadata.start_bursts = common::sdp::GetStartData((CONFIG_YADECAP_MBUFS_BURST_SIZE + 1) * sizeof(uint64_t), metadata.size);
	metadata.start_stats = common::sdp::GetStartData(sizeof(common::worker::stats::common), metadata.size);
	metadata.start_stats_ports = common::sdp::GetStartData(sizeof(common::worker::stats::port[CONFIG_YADECAP_PORTS_SIZE]), metadata.size);
