...
{
	common::fragmentation::stats_t total{};
	for (auto it : dataPlane->slow_workers)
	{
		total += it.second->Fragmentation().getStats();
	}
	return total;
}
...
{
	common::dregress::stats_t total{};
	for (auto it : dataPlane->slow_workers)
	{
		total += it.second->Dregress().Stats();
	}
	return total;
}
...
std::optional<std::reference_wrapper<const dataplane::sKniStats>> cControlPlane::KniStats(tPortId pid) const
{
	...
	for (auto it : dataPlane->slow_workers)
	{
		const auto& stats = it.second->KniWorker().PortStats(pid);
		if (stats)
		{
			return stats;
		}
...
{
	dataplane::hashtable_chain_spinlock_stats_t total{};
	for (auto it : dataPlane->slow_workers)
	{
		total += it.second->Dregress().Connections()->stats();
	}
	return total;
}
...
{
	dregress::LimitsStats total{};
	for (auto it : dataPlane->slow_workers)
	{
		total += it.second->Dregress().limits();
	}
	return total;
}
