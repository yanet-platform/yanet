...
void fw_config_t::set_macro(const std::string& s)
{
    ...
    m_macros.emplace(s, macro_t{
                            location_history_t{
                                (unsigned int)l.begin.line, m_fileno.top()},
                            {/* empty */}, false});
}
...
void fw_config_t::set_fqdn(const std::string& s)
{
    ...
    if (m_dns_cache.count(s) > 0)
    {
        ...
        m_dns_cache.emplace(s, dns_addresses_t{
                                   location_history_t{
                                       (unsigned int)l.begin.line, m_fileno.top()},
                                   {/* empty */}, false});
    }
...
void fw_config_t::create_iface_table(void)
{
    ...
    check_table();
    m_tables.emplace(m_curr_name, table_t{
                                      location_history_t{
                                          (unsigned int)l.begin.line, m_fileno.top()},
                                      tables::ifname_t{}});
}
...
void fw_config_t::check_table_entry(const tables::entry_type_t& e)
{
    ...
    {
        throw fw_parser_t::syntax_error(l, "attempt to add value of type '" +
                                               types[e.index()] + "' to table(" + m_curr_name + "), " +
                                               "expected type is '" + types[table.index()] + "'");
    }
}
