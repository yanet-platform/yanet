...
public:
	static void* CreateBufferInSharedMemory(const char* filename, uint64_t size, bool useHugeMem, bool useMemoryBySockets, tSocketId socket_id)
	{
		...
		int mode = 0644;
		YANET_LOG_WARNING("Run shm_open: %s\n", filename);
		...
		if (useMemoryBySockets)
		{
			...
			{
				YANET_LOG_ERROR("get_mempolicy(): %s\n", strerror(errno));
		...
		// Zero memory
		YANET_LOG_WARNING("Run memset: %p [%lu]\n", addr, size);
		std::memset(addr, 0, size);
		YANET_LOG_WARNING("After memset\n");
