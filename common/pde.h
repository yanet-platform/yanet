...
struct MainFileData
{
	                           ...
	                           bool useHugeMem)
	{
		...
		YANET_LOG_INFO("MainFileData::BuildFromDataPlane size main buffer: %lu\n", total_size);
	...
	{
		YANET_LOG_DEBUG("MainFileData::ReadFromDataplane start, open_sockets_file=%d\n", int(open_sockets_file));
		uint64_t size;
		...
		YANET_LOG_DEBUG("MainFileData::ReadFromDataplane finish, size=%ld\n", size);
