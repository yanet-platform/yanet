...
std::vector<std::tuple<std::string,
        ...
        commands = {{"help", "", [](const auto& args) { Call(help, args); }},
                    ...
                    {"proxy connections", "[service_name]", [](const auto& args) { Call(proxy::connections, args); }},
                    {"proxy syn", "[service_name]", [](const auto& args) { Call(proxy::syn, args); }},
                    {"proxy debug counter_id", "[service_id]", [](const auto& args) { Call(proxy::debug_counter_id, args); }},
                    {"proxy blacklist", "<service_name>", [](const auto& args) { Call(proxy::blacklist, args); }},
                    {"proxy blacklist add", "<service_name> <ip> <timeout>", [](const auto& args) { Call(proxy::blacklist_add, args); }},
