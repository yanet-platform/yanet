...
std::vector<std::tuple<std::string,
        ...
        commands = {{"help", "", [](const auto& args) { Call(help, args); }},
                    ...
                    {"physicalPort full", "", [](const auto& args) { Call(show::physicalPortsFull, args); }},
                    {"physicalPort full read", "", [](const auto& args) { Call(show::physicalPortsFullRead, args); }},
                    ...
                    {"proxy counters", "<proxy_ip> <proto> <proxy_port>", [](const auto& args) { Call(proxy::counters, args); }},
                    {"proxy tables", "<proxy_ip> <proto> <proxy_port>", [](const auto& args) { Call(proxy::tables, args); }},
                    {"proxy buckets", "<proxy_ip> <proto> <proxy_port>", [](const auto& args) { Call(proxy::buckets, args); }},
                    {"proxy bins", "<proxy_ip> <proto> <proxy_port>", [](const auto& args) { Call(proxy::bins, args); }},
                    {"proxy connections", "[proxy_ip] [proto] [proxy_port] <client_prefix>", [](const auto& args) { Call(proxy::connections, args); }},
                    {"proxy syn", "[proxy_ip] [proto] [proxy_port] <client_prefix>", [](const auto& args) { Call(proxy::syn, args); }},
                    {"proxy debug counter_id", "[service_id]", [](const auto& args) { Call(proxy::debug_counter_id, args); }},
                    {"proxy blacklist", "[proxy_ip] [proto] [proxy_port]", [](const auto& args) { Call(proxy::blacklist, args); }},
                    {"proxy blacklist add", "[proxy_ip] [proto] [proxy_port] [client_ip] [timeout]", [](const auto& args) { Call(proxy::blacklist_add, args); }},
                    ...
                    {"dontdoit podumoi tsc monitoring summary", "", [](const auto& args) { Call(develop::dataplane::tsc_monitoring_summary, args); }},
                    {},
                    ...
                    {"telegraf proxy", "", [](const auto& args) { Call(telegraf::proxy_counters, args); }},
                    {"telegraf proxy tables", "", [](const auto& args) { Call(telegraf::proxy_tables, args); }},
                    {},
                    ...
                    {"ringlog print", "<value>", [](const auto& args) { Call(ringlog::print, args); }},
                    {"ringlog print slow", "", [](const auto& args) { Call(ringlog::print_slow, args); }},
