...
TEST(ndarray, basic)
{
	...
	EXPECT_EQ(table.Values().size(), 12u);
...
TEST(ndarray, fill_and_get_2d)
{
	...
	size_t count = 0;
	table.ForEach([&](const Table::DimensionArray& keys, int value) {
...
TEST(ndarray, fill_and_get_3d)
{
	...
	EXPECT_EQ(table.Values().size(), 24);
...
TEST(ndarray, fill_and_clear_1d)
{
	...
	size_t count = 0;
	table.ForEach([&](const Table::DimensionArray& key, int value) {
	...
	table.Clear();
	EXPECT_TRUE(table.Values().empty()) << "Table should be empty after clear()";

	count = 0;
	table.ForEach([&](auto, auto) { count++; });
	EXPECT_EQ(count, 0u);
}
