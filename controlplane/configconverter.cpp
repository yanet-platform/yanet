...
void config_converter_t::processLogicalPorts()
{
	for (auto& [moduleName, logicalPort] : baseNext.logicalPorts)
	{
		...
		std::optional<tVrfId> vrfId = controlplane->getVrfIdsStorage().GetOrCreate(logicalPort.vrf);
		if (!vrfId.has_value())
		{
			throw error_result_t(eResult::invalidLogicalPortId, "invalid vrfId: " + std::to_string(logicalPort.vrfId));
		}
