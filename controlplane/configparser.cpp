                                                 ...
                                                 const std::map<std::string, nlohmann::json>& jsons)
{
	...
	try
	{
		...
		if (exist(rootJson, "host_config"))
		{
			loadConfig_host(baseNext.host_config, rootJson["host_config"]);
		}
	}
...
void config_parser_t::loadConfig_host(controlplane::base::host_t& host_config, const nlohmann::json& json)
{
	...
	if (exist(json, "hidden_real_ip"))
	{
		host_config.hidden_real_ip = json["hidden_real_ip"].get<bool>();
	}
}
