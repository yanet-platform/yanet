...
void network_table_t::clear()
{
	...
	group_id_to_filter_ids.clear();
	filter_id_to_group_ids_next.clear();
	group_id_to_filter_ids_next.clear();
	bitmask.clear();
}
...
void network_table_t::prepare(const uint32_t height, const uint32_t width)
{
	...
	filter_id_to_group_ids_next.resize(filter_ids.size());
}
...
void network_table_t::compile()
{
	     ...
	     filter_id++)
	{
		...
		const auto& network_ipv4_source_group_ids = compiler->network_ipv4_source.filter_to_group_ids[network_ipv4_source_filter_id];
		const auto& network_ipv4_destination_group_ids = compiler->network_ipv4_destination.filter_to_group_ids[network_ipv4_destination_filter_id];
		const auto& network_ipv6_source_group_ids = compiler->network_ipv6_source.filter_to_group_ids[network_ipv6_source_filter_id];
		const auto& network_ipv6_destination_group_ids = compiler->network_ipv6_destination.filter_to_group_ids[network_ipv6_destination_filter_id];
...
void network_table_t::Remap()
{
	     ...
	     layer_id++)
	{
		...
		{
			auto& remap_group_id = remap[network_table_group_id];
			if (!remap_group_id)
			{
	...
	for (unsigned int filter_id = 0;
	     ...
	     filter_id++)
	{
		for (const auto group_id : filter_id_to_group_ids[filter_id])
		{
			filter_id_to_group_ids_next[filter_id].emplace_back(remap[group_id]);
		}
	}
	filter_id_to_group_ids_next.swap(filter_id_to_group_ids);

	for (auto& [group_id, filter_ids] : group_id_to_filter_ids)
	{
		group_id_to_filter_ids_next[remap[group_id]].swap(filter_ids);
	}
	group_id_to_filter_ids_next.swap(group_id_to_filter_ids);

	for (auto& layer : compiler->transport.layers)
	{
		for (const auto group_id : layer.network_table_group_ids_vec)
		{
			layer.network_table_group_ids_vec_next.emplace_back(remap[group_id]);
		}
