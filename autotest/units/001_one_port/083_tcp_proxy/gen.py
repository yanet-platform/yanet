...
def packet_client_proxy(dt, raw='', tcp_options=[]):
	(seq, ack, flags) = parse_dt(dt, START_CLIENT_SEQ, 2000)
	return Ether(dst=MAC_PROXY, src=MAC_CLIENT)/Dot1Q(vlan=100)/IP(dst=IP_PROXY_EXT, src=IP_CLIENT1, ttl=63)/TCP(dport=80, sport=PORT_CLIENT, flags=flags, seq=seq, ack=ack, options=tcp_options)/Raw(raw)

def packet_server_proxy(dt, raw='', tcp_options=[]):
	(seq, ack, flags) = parse_dt(dt, 3000, START_CLIENT_SEQ)
	return Ether(dst=MAC_PROXY, src=MAC_SERVER)/Dot1Q(vlan=200)/IP(dst=IP_PROXY_INT, src=IP_SERVER, ttl=63)/TCP(sport=PORT_SERVER, dport=1025, flags=flags, seq=seq, ack=ack, options=tcp_options)/Raw(raw)

def packet_proxy_client(dt, ttl, window=8192, raw='', tcp_options=[]):
	(seq, ack, flags) = parse_dt(dt, 2000, START_CLIENT_SEQ)
	return Ether(dst=MAC_CLIENT, src=MAC_PROXY)/Dot1Q(vlan=100)/IP(dst=IP_CLIENT1, src=IP_PROXY_EXT, ttl=ttl)/TCP(dport=PORT_CLIENT, sport=80, flags=flags, seq=seq, ack=ack, window=window, options=tcp_options)/Raw(raw)

def packet_proxy_server(dt, ttl=63, raw='', tcp_options=[]):
